...
def get_single_student_features (name):
    ...
    featurized_person = person_df.drop(columns=['name', 'surname'])
...
def get_predctions_vector_full(models, person):
    """
...
def score_testing_dataset (model):
    ...
    """
    testing_dataset = [
            "Роман Арефьев", "Roman Arefev",
            "Светлана Кириченко", "Svetlana Kirichenko",
...
def score_testing_dataset_full(models):
    """
    ...
    data = []
    for name in testing_dataset:
        predictions = get_predctions_vector_full(models, name)
